...
void normalise_triangle(Triangle *tri) {
  for (int i = 0; i < 3; i++) {
    ...
    // printf("inside func 1: %f, %f, %f, %f", tri->vecs[0].x, tri->vecs[0].y,
    // tri->vecs[0].z, tri->vecs[0].w);
    ...
    // printf("inside func 2: %f, %f, %f, %f", tri->vecs[0].x, tri->vecs[0].y,
    // tri->vecs[0].z, tri->vecs[0].w);
  }
}
...
Mesh *get_square(double x, double y, double side, double z) {
  ...
  sq_mesh->tris[0] = create_triangle(create_vec4(x, y, z, W_DEF),
                                     create_vec4(x + side, y + side, z, W_DEF),
                                     create_vec4(x, y + side, z, W_DEF));

  sq_mesh->tris[1] = create_triangle(create_vec4(x, y, z, W_DEF),
